...
function PLUGIN:PostInstall(ctx)
    ...
    os.execute("mkdir -p " .. path .. "/bin")
    os.execute("mv " .. path .. "/" .. tool_name .. "-*/bin/* " .. path .. "/bin/")
    ...
    --[[
    -- Archive already contains bin/mise-dnf, just verify it works
    local testResult = os.execute(path .. "/bin/mise-dnf --version > /dev/null 2>&1")
    if testResult ~= 0 then
        error("mise-dnf installation appears to be broken")
    end
    --]]
