...
function PLUGIN:Available(ctx)
    ...
    local repo_url = "https://api.github.com/repos/" .. author .. "/" .. tool_name .. "/tags"
    ...
    -- -- repoquery : interroge les métadonnées des repos sans rien installer
    -- -- --qf : format de sortie
    -- -- 2>/dev/null : on ignore les warnings dnf éventuels
    -- local query = string.format(
    --     "dnf repoquery --quiet --qf '%%{version}-%%{release}\\n' %s 2>/dev/null | sort -V -u",
    --     tool
    -- )
    -- local result = cmd.exec(query)
    ...
    -- log.debug("Found " .. #versions .. " versions for " .. tool)
